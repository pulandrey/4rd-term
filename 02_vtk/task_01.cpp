...
public:
    ...
    void move(double tau, double time) {
        smth = pow(x, 3/2) * cos(100 * (x + y + z)) * cos(10 * time) + pow(y, 2) * sin(50 * (x + y + z)) * sin(20 * time) + pow(z, 2) * cos(200 * (x + y + z)) * cos(30 * time);
        // smth = (pow(x, 3.0/2.0) + y) * cos(100 * time) + (pow(y, 2) - z) * sin(150 * time) + (pow(z, 1) +x * y) * cos(200 * time);
        vx = (x > 20 && y < 17 && z > 140) ? pow(0, 1) : 0;
        vy = (x > 20 && y < 17 && z > 140) ? -10 * pow(7.45, 1.5) * cos(15 * time) : 0;
        vz = (x > 20 && y < 17 && z > 140) ? 10 * pow(5.22, 1.5) * cos(15 * time) : 0;

        vx = (x > 8 && y < 25 && z < 94.3 && z > 50) ? pow(0, 1) : (x > 20 && y < 17 && z > 140) ? pow(0, 1) : 0;
        vy = (x > 8 && y < 25 && z < 94.3 && z > 50) ? x * cos(50 * time) - y * sin(50 * time) : (x > 20 && y < 17 && z > 140) ? -1 * pow(7.45, 1.5) * cos(15 * time) : 0;
        vz = (x > 8 && y < 25 && z < 94.3 && z > 50) ? y * cos(50 * time) - x * sin(50 * time) : (x > 20 && y < 17 && z > 140) ? 1 * pow(5.22, 1.5) * cos(15 * time) : 0;
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = 0;
            // double vx = (pointX > 20 && pointY < 17 && pointZ > 140) ? pow(0, 1) : 0;
            // double vy = (pointX > 20 && pointY < 17 && pointZ > 140) ? -pow(7.45, 1) : 0;
            // double vz = (pointX > 20 && pointY < 17 && pointZ > 140) ? pow(5.22, 1) : 0;
            double vx = 0;
            double vy = 0;
            double vz = 0;
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "task_01-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    // Считаем STL
    try {
        gmsh::merge("/home/pulandrey/proga/4rd_term/02_vtk/unicorn-on-rock-sculpture-stl.stl");
    } catch(...) {
